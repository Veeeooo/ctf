...
class Article(Base):
    ...
    @classmethod
    def by_tag(cls,tag_id):
        article = dbSession.query(Tag).filter(Tag.id == tag_id).first()
    ...
    def by_category(cls,category_id):
        return dbSession.query(cls).filter_by(category_id=category_id).all()

    @classmethod
    def by_userid(cls,user_id):
